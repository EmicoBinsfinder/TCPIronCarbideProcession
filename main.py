...
EquilibriumFactor = [40, 99] # How many rows (out of 99) to ignore before calculating shear stress/friction coefficient, as it won't stabilise until after a certain number of timesteps
...
print("ln(A) at 1GPa is " + str(np.log(3.49 * (10 ** 9))))
print("ln(A) at 2GPa is " + str(np.log(2.86 * (10 ** 9))))
print("ln(A) at 3GPa is " + str(np.log(3.31 * (10 ** 9))))
print("ln(A) at 4GPa is " + str(np.log(3.16 * (10 ** 9))))
print("ln(A) at 5GPa is " + str(np.log(3.77 * (10 ** 9))))
...
err_E_1GPa = np.sqrt((4 * 1e9 * np.average(Average_Mu_List_4GPa) * error_actv * 1e-30) ** 2 + (1.381 * uncert_m * 1e-23) ** 2)

print("Activation_Energy Error Is " + str((float(err_E_1GPa)) * 6.02214076 * (10**23) / 1000))

pressures = ['2GPa', '3GPa', '4GPa', '5GPa']
plot_variation_in_shear_stress_constanttemp('500K', pressures)
...
RvN3.set_title('Log of Dissociation Rates vs Normal Stress')
