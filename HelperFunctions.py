...
def get_intact_columns_constant_temperature(Temperature, Pressures):
    ...
    All_Dataframe = pd.read_csv('F:/PhD/TCPDecompositionExperiments/Completed/FeC/{}/1GPa/'
...
def get_intact_columns_constant_pressure(Pressure, Temperatures):
    ...
    # Makes a dataframe from the intact molecuels csv of the first speed/temperature/pressure experiment in your defined list
    All_Dataframe = pd.read_csv('F:/PhD/TCPDecompositionExperiments/Completed/FeC/400K/{}/'
...
def get_fitted_plots_constant_temperature(BigDataframe, speed, Temperature):
    ...
    plt.legend()
    plt.xlabel('Time (ns)')
    plt.savefig('F:/PhD/TCPDecompositionExperiments/Completed/FeC/'
...
def plot_shear_stress_vs_normal_stress(Average_Shear_Stress_List_1, Average_Shear_Stress_List_2, Average_Shear_Stress_List_3, Temp1, Temp2, Temp3):
...
def format_fcdump_files(fcdump_file_path):
    Timestep = []
    v_s_bot = []
    v_p_bot = []
    file = pd.read_csv(fcdump_file_path)
    for x in file.iloc[1:, 0]:
        x = x.split(' ')
        Timestep.append(x[0])
        v_s_bot.append(x[1])
        v_p_bot.append(x[2])

    formatted_file = pd.DataFrame(Timestep, columns=['Timestep'])
    formatted_file['v_s_bot'] = v_s_bot
    formatted_file['v_p_bot'] = v_p_bot
    formatted_file.to_csv(fcdump_file_path, sep=' ', index=False)